...
type cacheEntry struct {
	...
	Daily map[string]map[string]codexlog.Usage `json:"daily"`
}
...
func expectedCacheMeta(opt BuildOptions, loc *time.Location) map[string]any {
	return map[string]any{
		...
		"timezone":      loc.String(),
		"parserVersion": codexlog.ParserVersion,
		"mtimeUnit":     "unix_nano",
	}
}
...
func BuildReport(cmd string, opt BuildOptions) (AggregatedReport, error) {
	...
	globalDaily := map[string]map[string]codexlog.Usage{}
...
func mergeDaily(global map[string]map[string]codexlog.Usage, daily map[string]map[string]codexlog.Usage) {
	for day, models := range daily {
		gm := global[day]
		if gm == nil {
			gm = map[string]codexlog.Usage{}
			global[day] = gm
		}
...
func buildRowsFromDaily(cmd string, daily map[string]map[string]codexlog.Usage, pr pricing.Pricing) []Row {
	switch cmd {
	case "monthly":
...
func buildDaily(daily map[string]map[string]codexlog.Usage, pr pricing.Pricing) []Row {
...
func buildMonthly(daily map[string]map[string]codexlog.Usage, pr pricing.Pricing) []Row {
	byMonth := map[string]map[string]codexlog.Usage{}
	for day, models := range daily {
		if len(day) < 7 {
			continue
		}
...
func cloneModels(in map[string]codexlog.Usage) map[string]codexlog.Usage {
	out := map[string]codexlog.Usage{}
	for k, v := range in {
		out[k] = v
	}
	return out

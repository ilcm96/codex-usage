...
func TestUsageNormalizer_TotalUsageSubtract(t *testing.T) {
	...
	first, ok := normalizer.NormalizeUsageLine([]byte(`{"timestamp":"2026-02-08T00:00:01.000Z","type":"event_msg","payload":{"type":"token_count","info":{"total_token_usage":{"input_tokens":100,"cached_input_tokens":20,"output_tokens":50,"reasoning_output_tokens":0,"total_tokens":150}}}}`))
	if !ok {
		t.Fatal("expected first usage event")
	}
	second, ok := normalizer.NormalizeUsageLine([]byte(`{"timestamp":"2026-02-08T00:00:02.000Z","type":"event_msg","payload":{"type":"token_count","info":{"total_token_usage":{"input_tokens":150,"cached_input_tokens":25,"output_tokens":70,"reasoning_output_tokens":0,"total_tokens":220}}}}`))
	if !ok {
	...
	if first.Usage.InputTokens != 100 || first.Usage.CachedInputTokens != 20 || first.Usage.OutputTokens != 50 || first.Usage.TotalTokens != 150 {
		t.Fatalf("unexpected first usage: %#v", first.Usage)
	}
	if second.Usage.InputTokens != 50 || second.Usage.CachedInputTokens != 5 || second.Usage.OutputTokens != 20 || second.Usage.TotalTokens != 70 {
